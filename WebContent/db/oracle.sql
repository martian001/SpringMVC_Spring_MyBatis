...
(
   ...
   STATUS               NUMBER,
   CREATOR_DATE       DATE,
   CREATOR_ID         VARCHAR2(55),
   UPDATE_ID          VARCHAR2(55),
   UPDATE_DATE        DATE,
...
(
   ...
   TYPE                 NUMBER,
   MODUEL               NUMBER,
   CONTENT              VARCHAR2(3048),
   IP_ADDRESS           VARCHAR2(50),
   OPERATOR_ID             VARCHAR2(55),
   CREATOR_DATE          TIMESTAMP
);
COMMENT ON TABLE SYS_LOG_INFO IS
'系统操作日志';
COMMENT ON COLUMN SYS_LOG_INFO.TYPE IS
'操作类型,1表示添加,2表示删除,3表示修改,4表示登录,5表示登出,6表示下载文件,7表示上传文件';
COMMENT ON COLUMN SYS_LOG_INFO.MODUEL IS
'日志模块：系统模块=1';
COMMENT ON COLUMN SYS_LOG_INFO.CONTENT IS
'操作内容';
COMMENT ON COLUMN SYS_LOG_INFO.IP_ADDRESS IS
'IP地址';
COMMENT ON COLUMN SYS_LOG_INFO.OPERATOR_ID IS
'操作人ID';
COMMENT ON COLUMN SYS_LOG_INFO.CREATOR_DATE IS
'创建日期';
